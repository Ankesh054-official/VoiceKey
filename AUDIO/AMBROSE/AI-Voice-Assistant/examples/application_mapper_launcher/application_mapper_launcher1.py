...
class InstalledApplications:
    ...
    def fetch_custom_app_paths(self):
        """Adds custom applications not listed in registry or Microsoft Store, like Android Studio."""
        common_paths = [
            r"C:\Program Files\Android\Android Studio\bin\studio64.exe",  # Default path for Android Studio
            # Add more paths if there are other known applications
        ]
        for path in common_paths:
            if os.path.exists(path):
                app_name = os.path.basename(path).split('.')[0]  # Get app name from path
                self.apps.append(Application(app_name, path))
