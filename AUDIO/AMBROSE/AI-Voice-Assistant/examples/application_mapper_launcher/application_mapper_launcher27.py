...
class Application:
    def __init__(self, name, path, is_store_app=False):
        self.name = name
        self.path = path
        self.is_store_app = is_store_app  # Flag for Microsoft Store or system apps
    ...
    def launch(self):
        ...
        try:
            if os.path.exists(self.path):
                ...
                print(f"Launching {self.name} directly via path.")
                os.startfile(self.path)
            elif self.is_store_app:
                # For Microsoft Store apps, use the AppsFolder path with package ID
                print(f"Launching {self.name} via AppsFolder...")
                store_path = f"explorer.exe shell:AppsFolder\\{self.path}"
                print(f"Store launch path: {store_path}")  # Debugging line to verify path
                subprocess.run(store_path, shell=True)
            else:
            ...
            if result.returncode == 0:
                ...
                for line in output_lines[3:]:  # Skip headers
                    ...
                    if len(parts) >= 2:
                        ...
                        # Ensure the correct format for AppsFolder launch
                        store_path = f"{package_name}!App"  # Correct format: package_name!App
                        print(f"Fetched Microsoft Store App: {app_name}, Package: {store_path}")  # Debugging line
                        self.apps.append(Application(app_name, store_path, is_store_app=True))
            else:
