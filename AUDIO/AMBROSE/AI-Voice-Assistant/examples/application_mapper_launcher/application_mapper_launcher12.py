...
class Application:
    def __init__(self, name, path, is_store_app=False):
        self.name = name
        self.path = path
        self.is_store_app = is_store_app  # Flag for Microsoft Store apps

    def launch(self):
        """Launch the application using the appropriate method."""
        try:
            if self.is_store_app:
                # Launch Microsoft Store apps with explorer.exe
                subprocess.run(["explorer.exe", self.path], shell=True)
            else:
                # Use os.startfile() for regular executables
