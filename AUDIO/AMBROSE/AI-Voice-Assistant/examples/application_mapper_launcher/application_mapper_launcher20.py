    ...
    def launch(self):
        ...
        try:
            if os.path.exists(self.path):
                ...
                # For Microsoft Store apps, use ms-appx-web:// URI
                print(f"Launching {self.name} via store URI...")
                # Format for launching Store apps: ms-appx-web://{PackageFullName}
                subprocess.run(f"start ms-appx-web://{self.path}", shell=True)
            else:
