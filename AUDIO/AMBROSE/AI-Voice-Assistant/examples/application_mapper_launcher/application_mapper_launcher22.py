    ...
    def launch(self):
        ...
        try:
            if os.path.exists(self.path):
                ...
                # For Microsoft Store apps, use PowerShell method
                print(f"Launching {self.name} via PowerShell...")
                subprocess.run(["powershell", "-Command", f"Start-Process 'shell:AppsFolder\\{self.app_id}'"])
            else:
