    ...
    def launch(self):
        ...
        try:
            if os.path.exists(self.path):
                ...
                # Using the ms-appx URI format for Microsoft Store apps
                print(f"Launching {self.name} using store URI...")
                subprocess.run(["start", f"ms-appx://{self.path}"], shell=True)  # Store apps URI format
            else:
                # For unknown applications, try with a URI format (e.g., ms-appx://)
            ...
            if result.returncode == 0:
                ...
                for line in output_lines[3:]:  # Skip headers
                    ...
                    if len(parts) >= 2:
                        ...
                        # Correct format to launch Store apps like Calculator
                        store_path = f"{package_name}!App"  # Format: package_name!App
