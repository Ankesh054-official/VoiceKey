...
class Application:
    ...
    def launch(self):
        try:
            # Attempt to launch using subprocess first
            subprocess.Popen([self.path])
            print(f"Launching {self.name}...")
        except PermissionError:
            # Fallback to os.startfile if subprocess fails with permission issues
            print(f"Permission error encountered. Trying os.startfile for {self.name}...")
            try:
                os.startfile(self.path)
                print(f"Launching {self.name} with os.startfile...")
            except Exception as e:
                print(f"Failed to launch {self.name} using os.startfile: {e}")
        except Exception as e:
